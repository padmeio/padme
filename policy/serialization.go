...
// MarshalJSON recursively serializes a RuleSet into a JSON object
...
// PartialRuleSet is a helper type that allows to partially deserialize a RuleSet so we can
...
// UnmarshalJSON recursively deserializes a RuleSet object
...
// PlainContents is a helper struct to Base64 encode the blob contents when serializing
type PlainContents struct {
	PluginID string `json:"plugin_id"`
	StrBlob  string `json:"blob"`
}

// MarshalJSON serializes a Contents object encoding the binary data in base64
...
// UnmarshalJSON deserializes a Contents object decoding the "blob" field
func (c *Contents) UnmarshalJSON(data []byte) error {
	plainContents := PlainContents{}
	if err := json.Unmarshal(data, &plainContents); err != nil {
		return err
	}
...
// MarshalJSON recursively serializes a PolicyLine object
...
// PartialPolicyLine is a hlper type that allows to partially deserialize a PolicyLine so we can
// use the standard JSON unmarshaler to deserialize the non-recursive objects.
...
// WrappedPolicyLine is a wrapper type to properly wrap and unwrap PolicyLine objects in
// a "policy_line" tag
type WrappedPolicyLine map[string]json.RawMessage

// UnmarshalJSON recursively deserializes a PolicyLine
func (pl *PolicyLine) UnmarshalJSON(data []byte) error {
	...
	if !ok {
		// No PolicyLine present; just return
		return nil
	}
...
// UnmarshalJSON deserialized a PolicyBundle. This custom marshaler is needed
// to properly deserialize the mixed type "policies" list
func (pb *PolicyBundle) UnmarshalJSON(data []byte) error {
	...
	var formatVersion, policyVersion int64
	if err := json.Unmarshal(raw["format_version"], &formatVersion); err != nil {
		return err
	}
	bundle.FormatVersion = uint64(formatVersion)
	if err := json.Unmarshal(raw["policy_version"], &policyVersion); err != nil {
		return err
	}
	bundle.PolicyVersion = uint64(policyVersion)

	if err := json.Unmarshal(raw["description"], &bundle.Description); err != nil {
		return err
	}

	var rawpolicies []json.RawMessage
	if err := json.Unmarshal(raw["policies"], &rawpolicies); err != nil {
		return err
	}
...
// readPolicy deserialuces a PolicyBase into a Policy object
...
// readPolicyLine deserialuces a PolicyBase into a PolicyLine object
